 ...
 * one is history, not today's board. Completed work remains visible with the
 * crew that performed it. */
const CALENDAR_STATUSES: AssignmentStatus[] = [
...
/**
 * A rerun can leave the published assignment in place while a replacement is
 * still a draft. Prefer that active planning version, then the most advanced
 * operational state. `updatedAt` and `id` make ties deterministic.
 */
const CALENDAR_STATUS_PRIORITY: Record<string, number> = {
  [AssignmentStatus.DRAFT]: 0,
  [AssignmentStatus.PROPOSED]: 1,
  [AssignmentStatus.IN_PROGRESS]: 2,
  [AssignmentStatus.ACKNOWLEDGED]: 3,
  [AssignmentStatus.PUBLISHED]: 4,
...
function toCalendarEntry(visit: VisitWithCalendarRelations): CalendarEntryDto {
  ...
  const assignment = selectCalendarAssignment(visit.assignments);
...
function selectCalendarAssignment(
  assignments: VisitWithCalendarRelations['assignments'],
): VisitWithCalendarRelations['assignments'][number] | null {
  return (
    [...assignments].sort((left, right) => {
      const statusDifference =
        (CALENDAR_STATUS_PRIORITY[left.status] ?? Number.MAX_SAFE_INTEGER) -
        (CALENDAR_STATUS_PRIORITY[right.status] ?? Number.MAX_SAFE_INTEGER);
